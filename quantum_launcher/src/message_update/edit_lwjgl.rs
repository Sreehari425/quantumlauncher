    ...
    pub fn update_edit_lwjgl(&mut self, message: EditLwjglMessage) -> Task<Message> {
        match message {
            ...
            EditLwjglMessage::VersionSelected(version) => {
                if let State::EditLwjgl(MenuEditLwjgl::Loaded { selected_version, mismatch_confirm, .. }) = &mut self.state {
                    ...
                    return Task::perform(
                        async move {
                            ...
                            for lib in &details.libraries {
                                let Some(name) = &lib.name else { continue; };
                            ...
                            let override_is_lwjgl3 = ql_core::json::lwjgl::is_lwjgl3(&selected_version);
            ...
            EditLwjglMessage::ApplyChecked(result) => {
                if let State::EditLwjgl(MenuEditLwjgl::Loaded { is_applying, mismatch_confirm, .. }) = &mut self.state {
                    *is_applying = false;
                    match result {
                        Ok(ApplyLwjglResult::Saved) => return Task::done(Message::EditLwjgl(EditLwjglMessage::Back)),
                        Ok(ApplyLwjglResult::NeedsConfirmation(msg)) => {
                            *mismatch_confirm = Some(msg);
                        }
                        Err(err) => return Task::done(Message::Error(format!("Failed to apply LWJGL version: {err}"))),
                    }
                }
                Task::none()
            }
            EditLwjglMessage::MismatchProceed => {
                let instance = self.instance().clone();
                if let State::EditLwjgl(MenuEditLwjgl::Loaded { selected_version, is_applying, mismatch_confirm, .. }) = &mut self.state {
                    *mismatch_confirm = None;
                    *is_applying = true;
                    let selected_version = selected_version.clone();
                    return Task::perform(
                        async move {
                            let mut config = ql_core::json::InstanceConfigJson::read(&instance).await?;
                            config.lwjgl_version = Some(selected_version).filter(|v| v != "default");
                            config.save(&instance).await?;
                            Ok(())
                        },
            ...
            EditLwjglMessage::MismatchRevert => {
                if let State::EditLwjgl(MenuEditLwjgl::Loaded { selected_version, initial_version, mismatch_confirm, is_applying, .. }) = &mut self.state {
                    *is_applying = false;
                    *mismatch_confirm = None;
                    *selected_version = initial_version.clone().unwrap_or_else(|| "default".to_owned());
                }
                Task::none()
            }
