...
use crate::tui::app::{App, AddAccountFieldFocus};
    ...
    pub fn handle_auth_event(&mut self, event: crate::tui::AuthEvent) {
        match event {
            ...
            crate::tui::AuthEvent::LoginSuccess { account_data } => {
                ...
                let account = Account { username: account_data.nice_username.clone(), account_type: account_data.account_type.to_string(), uuid: account_data.uuid.clone(), is_logged_in: true };
            ...
            crate::tui::AuthEvent::LaunchStarted(instance_name) => {
                ...
                self.add_log(format!("[{}] Launch started for instance: {}", chrono::Local::now().format("%H:%M:%S"), instance_name));
            }
            crate::tui::AuthEvent::LaunchSuccess(instance_name, child) => {
                ...
                let client_process = crate::state::ClientProcess { child: child.clone(), receiver: None };
                ...
                {
                    let (stdout, stderr) = { let mut guard = child.lock().unwrap(); (guard.stdout.take(), guard.stderr.take()) };
                    if let Some(mut out) = stdout {
                        ...
                        tokio::spawn(async move {
                            ...
                            while let Ok(Some(line)) = lines.next_line().await {
                                let mut s = line; if !s.ends_with('\n') { s.push('\n'); }
                                if let Some(tx) = &ui_tx { let _ = tx.send(crate::tui::AuthEvent::InstanceLogLine { instance_name: inst_name.clone(), line: s }); }
                            }
                        });
                    }
                ...
                if let Some(instance) = self.instances.iter_mut().find(|i| i.name == instance_name) { instance.is_running = true; }
                        ...
                        loop {
                            {
                                ...
                                if let Ok(Some(_)) = child_guard.try_wait() { break; }
                            }
            ...
            crate::tui::AuthEvent::LaunchEnded(instance_name) => {
                ...
                if let Some(instance) = self.instances.iter_mut().find(|i| i.name == instance_name) { instance.is_running = false; }
            ...
            crate::tui::AuthEvent::InstanceLogLine { instance_name, line } => {
                ...
                if self.current_tab == crate::tui::app::TabId::InstanceSettings
                    ...
                    && self.instance_logs_auto_follow { self.instance_logs_offset = 0; }
            }
            crate::tui::AuthEvent::InstanceCreateStarted(instance_name) => {
                ...
                self.is_loading = true;
                self.add_log(format!("[{}] Started creating instance: {}", chrono::Local::now().format("%H:%M:%S"), instance_name));
            }
            crate::tui::AuthEvent::InstanceCreateProgress { instance_name, message } => {
                ...
                self.add_log(format!("[{}] Instance '{}': {}", chrono::Local::now().format("%H:%M:%S"), instance_name, message));
            }
            ...
            crate::tui::AuthEvent::RefreshStarted => {
                ...
                self.is_loading = true;
                self.add_log(format!("[{}] Started refreshing instances", chrono::Local::now().format("%H:%M:%S")));
            }
            crate::tui::AuthEvent::RefreshCompleted => {
                self.is_loading = false;
                self.status_message = "✅ Instances refreshed successfully".to_string();
                self.add_log(format!("[{}] Instances refresh completed", chrono::Local::now().format("%H:%M:%S")));
            }
            crate::tui::AuthEvent::RefreshData { instances } => {
                ...
                if self.selected_instance >= self.instances.len() && !self.instances.is_empty() { self.selected_instance = self.instances.len() - 1; }
                else if self.instances.is_empty() { self.selected_instance = 0; }
            }
        }
    }
}
