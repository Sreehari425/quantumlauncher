...
impl App {
    ...
    pub const fn licenses_menu_index() -> usize { crate::tui::app::licenses::menu_index() }
    ...
    pub fn settings_next_item(&mut self) {
        ...
        if self.settings_focus == SettingsFocus::Middle && self.about_selected == Self::licenses_menu_index() {
            ...
            self.about_selected = if left_count > 0 { (self.about_selected + 1) % left_count } else { 0 };
            self.about_scroll = 0;
        }
    }
    ...
    pub fn settings_prev_item(&mut self) {
        if self.settings_focus == SettingsFocus::Middle && self.about_selected == Self::licenses_menu_index() {
            ...
            if self.about_selected > 0 { self.about_selected -= 1; } else { self.about_selected = left_last; }
            self.about_scroll = 0;
        }
    }
}
